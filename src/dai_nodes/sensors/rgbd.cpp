...
namespace dai_nodes {
RGBD::RGBD(const std::string &daiNodeName, std::shared_ptr<rclcpp::Node> node,
           std::shared_ptr<dai::Pipeline> pipeline,
           std::shared_ptr<dai::Device> device, bool rsCompat,
           SensorWrapper &camNode,
           std::shared_ptr<dai::node::StereoDepth> stereo, bool aligned)
  ...
  ph = std::make_unique<RGBDParamHandler>(node, daiNodeName,
                                          device->getDeviceName(), rsCompat);
  ...
  // rgbdNode->runOnHost(ph->getParam<bool>("i_run_sync_on_host"));
  auto fps = ph->getOtherNodeParam<float>(camNode.getName(), ParamNames::FPS);

  auto *out = color->requestOutput(
      std::pair<int, int>(
          ph->getOtherNodeParam<int>(camNode.getName(), ParamNames::WIDTH),
          ph->getOtherNodeParam<int>(camNode.getName(), ParamNames::HEIGHT)),
      dai::ImgFrame::Type::RGB888i, dai::ImgResizeMode::CROP, fps, true);
  ...
  if (platform == dai::Platform::RVC4) {
    if (!aligned) {
      ...
      RCLCPP_DEBUG(getLogger(),
                   "Stereo depth output is reported to be aligned. Please "
                   "connect its output externally");
    }
  } else {
    if (!aligned) {
...
RGBD::RGBD(const std::string &daiNodeName, std::shared_ptr<rclcpp::Node> node,
           std::shared_ptr<dai::Pipeline> pipeline,
           std::shared_ptr<dai::Device> device, bool rsCompat,
           SensorWrapper &camNode, ToF &tofNode, bool aligned)
    : BaseNode(daiNodeName, node, pipeline, device->getDeviceName(), rsCompat) {
  using namespace param_handlers;
  RCLCPP_DEBUG(getLogger(), "Creating node %s", daiNodeName.c_str());
  setNames();
  ...
  if (!aligned) {
    ...
    RCLCPP_DEBUG(getLogger(), "Depth has been prealigned! Please remember to "
                              "link manually in pipeline creation.");
  }
...
void RGBD::setupQueues(std::shared_ptr<dai::Device> /* device */) {
  ...
  pclQ = rgbdNode->pcl.createOutputQueue(
      ph->getParam<int>(ParamNames::MAX_Q_SIZE), false);
  auto tfPrefix = getOpticalFrameName(getSocketName(ph->getSocketID()));
  rclcpp::PublisherOptions options;
  options.qos_overriding_options = rclcpp::QosOverridingOptions();
  pclConv = std::make_unique<depthai_bridge::PointCloudConverter>(
      tfPrefix, ph->getParam<bool>(ParamNames::GET_BASE_DEVICE_TIMESTAMP));
  pclConv->setUpdateRosBaseTimeOnToRosMsg(
      ph->getParam<bool>(ParamNames::UPDATE_ROS_BASE_TIME_ON_ROS_MSG));
  pclConv->setDepthUnit(
      dai::StereoDepthConfig::AlgorithmControl::DepthUnit::METER);

  pclPub = getROSNode()->create_publisher<sensor_msgs::msg::PointCloud2>(
      "~/" + getName() + "/points", ph->getParam<int>(ParamNames::MAX_Q_SIZE),
      options);
  cbID = pclQ->addCallback(std::bind(&RGBD::pclCB, this, std::placeholders::_1,
                                     std::placeholders::_2));
}
...
void RGBD::pclCB(const std::string & /*name*/,
